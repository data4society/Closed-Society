;(function(Form) {
  ...
  Form.editors.List = Form.editors.Base.extend({
    ...
    render: function() {
      ...
      //Create main element
      var $el = $($.trim(this.template({title: self.schema.title})));
      ...
      if (value.length) {
        _.each(value, function(itemValue) {
        	//itemValue.organization = itemValue.organization
  ...
  Form.editors.List.Modal = Form.editors.Base.extend({
    ...
    itemToString: function(value) {
      ...
      _.each(this.nestedSchema, function(schema, key) {
        ...
        if (schema.type=='Datepicker' && !(_.isNull(val)) && val != '') {
        	if(schema.time) {
        		val = moment(val,'X').format('DD.MM.YYYY HH:mm');
        	}
        	else {
        		val = moment(val,'X').format('DD.MM.YYYY');
        	}
        }
        else if (schema.type=='Select2RelObject') {
